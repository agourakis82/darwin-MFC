    ...
    <section className="overflow-hidden rounded-[32px] border shadow-[0_18px_48px_rgba(0,0,0,0.18)]" style={{ background: 'rgba(255,255,255,0.035)', borderColor: alert.border }}>
      ...
      <div className="space-y-4 px-5 py-5">
        <div>
          ...
          <p className="ps-app-kicker">Tarefa ativa</p>
          <h2 className="mt-1.5 text-[1.85rem] font-bold leading-[1.02] tracking-[-0.05em] text-white md:text-[2.15rem]">{step.title}</h2>
          <p className="mt-2.5 max-w-3xl text-sm leading-relaxed text-slate-400 md:text-[14px]">{step.description}</p>
        </div>
        ...
        <div className="rounded-[28px] border border-white/8 bg-[#08111d]/82 p-4 md:p-5">
          ...
          {step.type === 'drug' && step.drugId && (
            <button
              ...
              className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[20px] border border-amber-400/22 bg-amber-500/10 px-4 py-3 text-sm font-semibold text-amber-100 transition-all duration-150 ease-out hover:bg-amber-500/14"
            >
          ...
          {step.type === 'timer' && (
            <button
              type="button"
              onClick={onOpenTimer}
              className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[20px] border border-rose-400/20 bg-rose-500/10 px-4 py-3 text-sm font-semibold text-rose-100 transition-all duration-150 ease-out hover:bg-rose-500/14"
            >
          ...
          {step.type === 'score' && step.scoreId && (
            <Link
              ...
              className="inline-flex min-h-[52px] items-center justify-center gap-2 rounded-[20px] border border-cyan-400/20 bg-cyan-400/10 px-4 py-3 text-sm font-semibold text-cyan-100 transition-all duration-150 ease-out hover:bg-cyan-400/14"
            >
        ...
        {step.notes?.length ? (
          <div className="rounded-[22px] border border-white/8 bg-white/[0.03] px-4 py-4 shadow-[inset_0_1px_0_rgba(255,255,255,0.02)]">
            ...
            <div className="space-y-2">
              {step.notes.map((note) => (
                <div key={note} className="rounded-[16px] border border-white/7 bg-white/[0.025] px-3 py-2">
        ...
        {step.type === 'decision' && step.options?.length ? (
          <div className="grid gap-3 md:grid-cols-2">
            {step.options.map((opt, i) => (
              <button
                ...
                className="rounded-[22px] border border-white/8 bg-white/[0.035] px-4 py-4 text-left transition-all duration-150 ease-out hover:bg-white/[0.05] hover:-translate-y-0.5"
              >
          ...
          <button
            ...
            onClick={onAdvance}
            disabled={isLastStep}
            className="flex w-full items-center justify-center gap-2 rounded-[26px] px-4 py-3.5 text-sm font-bold transition-all duration-150 ease-out shadow-[0_18px_40px_rgba(0,0,0,0.16)]"
            style={isLastStep
