...
export default function PSProtocolSafetySidebar({
  ...
  return (
    <aside className="space-y-3 xl:sticky xl:top-24">
      <div className="rounded-[24px] border border-white/10 bg-[#08111d]/86 p-4">
        ...
        <div className="rounded-[20px] border border-white/8 bg-white/[0.03] px-4 py-4">
          ...
          <div className="mt-4 space-y-2 text-sm">
            <div className="flex items-center justify-between rounded-[16px] border border-white/7 bg-white/[0.03] px-3 py-2.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Etapa</span>
              <span className="truncate pl-3 font-semibold text-white">{currentStepId}</span>
            </div>
            <div className="flex items-center justify-between rounded-[16px] border border-white/7 bg-white/[0.03] px-3 py-2.5">
              <span className="text-[10px] font-bold uppercase tracking-wider text-slate-500">Eventos</span>
              <span className="font-semibold text-white">{timelineEvents.length}</span>
            </div>
          </div>
        </div>
      </div>
      ...
      {protocolId === 'pcr' && pcrPausePoints.length > 0 && (
        <div className="rounded-[22px] border border-rose-500/16 bg-rose-500/[0.05] p-4 space-y-3">
          ...
          <div className="space-y-2">
            {pcrPausePoints.map((point) => (
              <div key={`${currentStepId}-${point}`} className="rounded-[14px] border border-white/8 bg-black/10 px-3 py-3">
